...
{
  int hartid = read_tp();
  char *pathpa = (char *)user_va_to_pa((pagetable_t)(current[hartid]->pagetable), pathva); // 得到物理地址z
  // sprint("pathpa:%s\n",pathpa);
...
{
  //  sprint("sp:%lx,s0:%lx", current[hartid]->trapframe->regs.sp, current[hartid]->trapframe->regs.s0);
